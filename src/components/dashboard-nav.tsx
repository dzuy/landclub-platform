...
export function DashboardNav({showAdminTools}:{showAdminTools:boolean}){
 ...
 return <aside className={`cms-sidebar dashboard-sidebar${open?' is-open':''}`}>
  ...
  <div className="dashboard-sidebar-panel" id="dashboard-navigation">
   ...
   <nav aria-label="Your Land Club">{links.map(([label,href,icon])=><Link key={href} href={href} aria-current={path===href?'page':undefined} onClick={()=>setOpen(false)}><NavIcon name={icon}/><span>{label}</span></Link>)}{showAdminTools&&<><div className="dashboard-nav-divider"/><div className="eyebrow">ADMIN TOOLS</div><Link href="/staff/members" aria-current={path==='/staff/members'?'page':undefined} onClick={()=>setOpen(false)}><NavIcon name="members"/><span>Members</span></Link><Link href="/staff/properties" aria-current={path.startsWith('/staff/properties')?'page':undefined} onClick={()=>setOpen(false)}><NavIcon name="content"/><span>Property content</span></Link></>}</nav>
  </div>
 </aside>
}
